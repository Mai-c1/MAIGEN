<template>
  <div class="module-container">
    ...
    >
      <template #columns>
        <a-table-column title="UID" data-index="id" :width="80" />
        ...
        <a-table-column title="资产余额">
          <template #cell="{ record }">
            <div class="flex flex-col">
              <span class="font-bold text-yellow-500">{{ (record.points || 0).toLocaleString() }}</span>
            </div>
          </template>
        </a-table-column>
...
import { IconPlus, IconUser, IconSettings } from '@arco-design/web-vue/es/icon';
...
const searchItems = [
  { field: 'keyword', label: '搜索', type: 'input', placeholder: '用户名/邮箱' },
  { 
    field: 'status', 
    label: '状态', 
    type: 'select', 
  ...
  { field: 'minPoints', label: '最小积分', type: 'input', placeholder: '0' },
  { field: 'maxPoints', label: '最大积分', type: 'input', placeholder: '99999' }
];
